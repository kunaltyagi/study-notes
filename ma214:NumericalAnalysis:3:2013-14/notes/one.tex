...
\begin{itemize}
    ...
    \item \textbf{Cholesky's Algorithm}: For a Positive definite n$\times$n matrix, A=$LL^T$ where L is a Lower Triangular matrix. 
    \begin{itemize}
        \item To find L, use forward-substitution in A=L$L^T$
    ...
    \item PA permutes the rows of A. Also $P^{-1}$ exists and is equal to $P^T$
    \item \textbf{PLU factorization}:
\end{itemize}
\end{answer}
