...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \]
    \end{itemize}
    \item Richardson Extrapolation: $M = N(h) + k_1h + k_2h^2 + k_3h^3 \ldots$ with unknown $k_i$ and all h as h $\to$ 0\\
    ...
    \begin{itemize}
        ...
        \item O($h^i$) approx: M$\approx N_i$(h) = $N_{i-1} + \frac{N_{i-1}(h/2) - N_{i-1}(h)}{2^{j-1} - 1}$
    \end{itemize}
